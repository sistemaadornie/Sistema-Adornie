# Redesign Visual da Tela de Editar Pedido — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reestruturar `EditarPedido.jsx` para usar o sistema visual já existente (`os-form-section`, `os-info-bar`, `os-field`, `os-textarea`, definidos em `OrdemServicoModal.css`, já importado pelo arquivo), dando à tela a mesma hierarquia visual das Fichas de Confecção/Conferência — sem CSS novo e sem mudança de comportamento.

**Architecture:** É uma reescrita do JSX de um único arquivo (`EditarPedido.jsx`), trocando `pf-form-field`/divs soltas por cartões `os-form-section` com título, adicionando uma barra de contexto (`os-info-bar`) no topo com Cliente/Pedido/Vendedor/Arquiteto, e carregando dois campos a mais (`cliente_nome`, `pedido_numero`) do mesmo objeto de resposta já buscado. Toda a lógica (carregar, salvar, editar itens/pagamentos, abrir PDF, voltar) permanece idêntica.

**Tech Stack:** React (`frontend-web/src`). Sem mudanças de backend.

## Global Constraints

- Nenhuma mudança de comportamento do formulário — mesmos campos, mesma ausência de validação client-side, mesmo fluxo de salvar/cancelar/voltar.
- Nenhum CSS novo — só classes já definidas em `OrdemServicoModal.css` (`os-form-section`, `os-section-title`, `os-field`, `os-info-bar`, `os-info-row`, `os-info-item`, `os-info-item-grow`, `os-info-label`, `os-info-value`, `os-info-value.tag-pedido`, `os-textarea`, `os-grid-2`).
- Tabelas de Itens (`pd-itens-editor`) e Pagamentos (`pd-pag-editor`) não mudam por dentro — só ganham um cartão `os-form-section` ao redor.
- `cliente_nome`/`pedido_numero` vêm do mesmo `resPedido.pedido` já buscado em `carregar()` — nenhuma chamada de API nova.
- Sem testes automatizados de frontend neste projeto (nenhum `*.test.jsx` existe) — verificação via build + lint, e teste manual no navegador.

---

### Task 1: Reescrever `EditarPedido.jsx` com o novo visual

**Files:**
- Modify: `frontend-web/src/pages/pedidos/EditarPedido.jsx`

**Interfaces:**
- Nenhuma — arquivo autocontido, sem consumidores além da rota `/pedidos/:id/editar` já registrada em `App.jsx` (inalterada).

- [ ] **Step 1: Substituir o arquivo inteiro pelo conteúdo abaixo**

Substituir todo o conteúdo de `frontend-web/src/pages/pedidos/EditarPedido.jsx` por:
...
Run: `cd frontend-web && npx eslint src/pages/pedidos/EditarPedido.jsx`
...
1. Abrir a tela de Editar Pedido de um pedido existente (`/pedidos/:id/editar`, ou pelo botão "✏️ Editar Pedido" na Etapa 1 do fluxo) — confirmar que aparece a barra de contexto no topo com Cliente, Pedido, Vendedor e Arquiteto preenchidos corretamente.
2. Confirmar que "Dados do Pedido", "Endereço de Entrega" e "Observações" aparecem dentro de cartões com título, com o mesmo visual das Fichas de Confecção (bordas, fundo, foco destacado ao clicar num campo).
3. Confirmar que "Itens (N)" e "Pagamentos (N)" mostram a contagem correta no título e que adicionar, remover e editar linhas continua funcionando exatamente como antes.
4. Editar um campo (ex: Status) e clicar "Salvar alterações" — confirmar que volta para o fluxo do pedido com a Etapa 1 reaberta e o dado atualizado.
5. Alternar entre tema claro e escuro (o app tem os dois) e confirmar que os cartões/campos têm contraste adequado nos dois.

- [ ] **Step 5: Commit**

```bash
git add frontend-web/src/pages/pedidos/EditarPedido.jsx
git commit -m "feat(pedidos): redesenha visualmente a tela de Editar Pedido"
```
...
- **Cobertura da spec:** Barra de contexto (2.1), Dados do Pedido (2.2), Endereço de Entrega (2.3), Observações (2.4), Itens (2.5) e Pagamentos (2.6) — todos presentes no arquivo reescrito. Novo dado carregado (`cliente_nome`, `pedido_numero`, seção 1 da spec) incluído no `setForm`.
- **Sem CSS novo:** todas as classes usadas (`os-form-section`, `os-section-title`, `os-field`, `os-info-bar`, `os-info-row`, `os-info-item`, `os-info-item-grow`, `os-info-label`, `os-info-value`, `tag-pedido`, `os-textarea`, `os-grid-2`, `os-btn`, `os-btn-secondary`, `os-btn-primary`, `os-back-btn`, `os-page-title`, `os-alert`, `os-alert-danger`) já existem em `OrdemServicoModal.css` e já são usadas por outras telas (`FichaConfeccaoCortina.jsx` etc.) — nenhuma classe nova precisa ser criada.
- **Sem mudança de comportamento:** `carregar`, `set`, `setItem`, `addItem`, `removeItem`, `setPag`, `addPag`, `removePag`, cálculo de `subtotal`/`desconto`/`total`, `voltar`, `abrirPdf`, `salvar` são transcrições byte-a-byte do arquivo atual — só a função `carregar` ganhou duas linhas a mais no `setForm` (`cliente_nome`, `pedido_numero`) e o `return` (JSX) foi reestruturado.
- **Sem placeholders:** o arquivo completo está no Step 1, pronto para transcrição.
